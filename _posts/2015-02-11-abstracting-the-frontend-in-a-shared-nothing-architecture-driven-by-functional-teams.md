...
category: general
author: ricopfaus
tags: [javascript,coffeescript,tracking,frontend]
---
{% include JB/setup %}

## The challenge
Before I explain what exactly a "frontend middleware" is, let me first tell you why we invented it and what it is good for. 

About one year ago, when I joined the new e-commerce team at GALERIA Kaufhof working on their new online platform, everything was still in its early stages. The idea of having totally separated application "verticals" was the crazy new thing and the entire system was cut into microservices to keep the business logic separated. From a backend and business perspective this perfectly made sense.
...
This technique felt ideal to approach our separation issue and (re-)unify the tracking-related topics within one functional team again (mine ;-) ...). We wanted to take it even one step further and do fancy things like declarative tracking, channel recognition and tag management within our own context. Sounds cool? It is.
...
To avoid inlined script tags whenever possible we decided to not rely on Javascript for passing the data around. Instead we use dedicated metatags containing JSON data for passing information from the markup to our application layer (which btw we called "Data Abstraction Layer" or DAL). An example metatag could look like this:
...
This also gave us the great advantage that each vertical application could independently render any data right into any page's markup without worrying about availability or load order of a library or API. Markup loads reliably earlier than scripts in most common browsers ;-) ... For asynchronously loaded content we developed another (Javascript-driven) solution based on broadcasting events top-down from the DAL to its plugins. I'll talk about that later.
...
The plugin loading is based on a simple ruleset which, optionally, executes callbacks before loading a plugin. That allows for very advanced serve rule logic, which no tagmanager could offer out-of-the-box (e.g. *if the page's type is "homepage" and the user is logged in*). This even enables us to eventually replace our external tagmanager and host the entire affiliate integration right within our own git repository. If you are a developer and/or you ever worked with external tag management GUIs (or any other, less comfortable form of affiliate integration) you might know what a great relief that is. We are even able to **fully unit test our affiliate pixels, integrated within our CD pipeline**. Getting jealous? You should! ;-)    
...
define "ba/lib/dal/demoAffiliate", ["jquery", "thirdpartylib"] ($, thirdpartyLib) ->
...
## Conclusion
So, finally, what is a "frontend middleware"? I introduced this term to describe a software architecture that "sits" between client and third-party space and provides a complete abstraction between those two. (Can anyone please put that on Wikipedia? :-D ..) It replaces common tagmanagers, affiliate pixels and alike. At the same time it provides a declarative tracking API and deep integration with RUM, web analytics and other frontend-only tools commonly served by third parties (surveyforms, promolayers, onsite A/B testing ...). It is primarily aimed at verticalized shared-nothing architectures that have no monolithic frontend, but should also play well with any other architecture, too.

Why is this so incredibly cool? Because its my own idea ;-) ... No, seriously, this has a few major advantages over common approaches: 

1. It is almost unblockable by adblockers, because it directly integrates with our codebase. Especially when combined with an integrated RUM solution this offers an invaluable advantage over commercial tools that can be easily blocked.
2. It offers extermely fine-grained control over serve rules and any data you hand over to pixels and/or snippets.   
2. All pixels, snippets and alike are committed to a code repository and the developers can work with their everyday tools and editors to work on the code.
3. (This is my favourite) It is fully unit-testable and integrated into the CI/CD pipeline. No more broken affilaite pixels and endless ping-pong with online marketing units to find problems with the integration (do you feel the pain?). It requires a little more discipline to receive the correct acceptance criteria, but it pays out ten times in the end.
4. What matters most - it makes you the damn coolest kid on the block ;-) 

And the best of all? It will be open-sourced soon. (... and everybody was like: "Yaaaaay!")
